defmodule AiDrudge.Agent do
  ...
  def request(message, model \\ "anthropic:claude-3-5-sonnet") do
    schema = [
      republican: [type: :string, required: true],
      democrat: [type: :string, required: true],
      key_points: [type: :list, required: true],
      support: [side: :string, required: true]
    ]
    ...
    ReqLLM.Response.object(response) |> AiDrudge.Articles.create_article()
